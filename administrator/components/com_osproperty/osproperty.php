...
if($maintask != "ajax"){
	$blacktaskarry = array('properties_showphotosinzipfile','properties_print','extrafield_addfieldoption','extrafield_removefieldoption','extrafield_savechangeoption','upload_ajaxupload','agent_getstate');
...
switch ($maintask){
	...
	break;
	case "fieldgroup":
		if (!JFactory::getUser()->authorise('extrafield_groups', 'com_osproperty')) {
	...
	break;
	case "type":
		if (!JFactory::getUser()->authorise('property_types', 'com_osproperty')) {
			return JError::raise(E_WARNING, 404, JText::_('JERROR_ALERTNOAUTHOR'));
		}
