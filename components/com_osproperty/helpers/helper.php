...
class OSPHelper
{
    ...
    public static function loadBootstrap($loadJs = true)
    {
        ...
        if ($configClass['load_bootstrap_adv'] == 1) {
            if (file_exists(JPATH_ROOT .'/templates/'. $current_template . '/html/com_osproperty/css/bootstrap_adv.css')) {
                $document->addStyleSheet(JURI::root() .'templates/'. $current_template.'/html/com_osproperty/css/bootstrap_adv.css');
            }else {
                $document->addStyleSheet(JURI::root() . 'components/com_osproperty/js/bootstrap/css/bootstrap_adv.css');
            }
        }
    	if ($language->isRTL()) {
		   $document->addStyleSheet(JURI::root() . 'components/com_osproperty/js/bootstrap/css/bootstrap_rtl.css');
		}
    }
    ...
    public static function loadBootstrapStylesheet()
    {
        ...
        $document = JFactory::getDocument();
        $document->addStyleSheet(JURI::root() . 'components/com_osproperty/js/bootstrap/css/bootstrap.css');
       // $document->addStyleSheet(JURI::root() .'/templates/'. 'components/com_osproperty/js/bootstrap/css/bootstrap-responsive.css');
        if ($configClass['load_bootstrap_adv'] == 1) {
    ...
    public static function loadGoogleJS(){
        ...
        if($loadGoogleJs == false){
            $document = JFactory::getDocument();
            $document->addScript("//maps.googleapis.com/maps/api/js?sensor=false");
            $loadGoogleJs = true;
        }
    }
...
//		$query = 'SELECT *' .
	...
	static function registerNewAgent($user,$user_type){
		global $mainframe;
		if($user->id == 0){
			$user = JFactory::getUser();
		}
