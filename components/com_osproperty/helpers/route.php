		...
		if(count($needs) > 0){
			...
			if($needs[0] == "property_details"){
				...
				if($pid > 0){
					...
					$db->setQuery("Select id from #__menu where published = '1' and (`link` like '%view=ldetails%' and `link` like '%id=".$pid."%') $language_sql and `access` IN (". implode(',', $user->getAuthorisedViewLevels()) .")");
