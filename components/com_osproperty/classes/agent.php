...
class OspropertyAgent{
	...
	static function editProfile($option){
		...
		if(!HelperOspropertyCommon::isAgent()){
			$agent_id = OSPHelper::registerNewAgent($user);
		}
		$document = JFactory::getDocument();
	...
	static function submitContact($option,$id){
		...
		$mainframe->redirect(JRoute::_("index.php?option=com_osproperty&task=agent_info&id=".$id."&Itemid=".JRequest::getInt('Itemid')),JText::_('OS_EMAIL_HAS_BEEN_SENT'));
	}
	...
	static function completeRegistration($option){
		...
		if(intval($user->id) == 0){
            ...
            $registerReturn = OSPHelper::registration($data);
		...
		$agent_id = OSPHelper::registerNewAgent($user, JRequest::getInt('agent_type',-1));
