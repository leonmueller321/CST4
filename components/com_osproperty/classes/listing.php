...
class OspropertyListing{
	...
	static function propertyCity($option,$city_id){
		...
		if (is_object($menu)) {
			...
			if(count($catIds) == 0){
				...
				if(count($catIds) == 0) {
					JRequest::setVar('catIds',$category_id);
				}
			}
	...
	static function propertyType($option){
		...
		$submitCategory				= JRequest::getInt('submitCategory',0);
		
		//if($submitCategory == 0){
			//$catIds = $session->get('catIds');
		//}else{
			//$session->set('catIds',null);
		//}
		...
		$state_id					= JRequest::getInt('state_id',-1);
		if($state_id == -1){
			$state_id = $session->get('state_id');
		}
		$agenttype					= JRequest::getInt('agent_type',-2);
		if($agenttype == -2){
			$agenttype = $session->get('agenttype',-1);
		}
		//print_r($session);
		//$catIds						= JRequest::getVar('category_id',array());
		
		$type_id					= JRequest::getInt('property_type',-1);
		if($type_id == -1){
			$type_id = $session->get('type_id');
		}

		$min_price					= JRequest::getFloat('min_price',-1);
		if($min_price == -1){
			$min_price = $session->get('min_price');
		}
		$max_price					= JRequest::getFloat('max_price',-1);
		if($max_price == -1){
			$max_price = $session->get('max_price');
		}
		$price					= JRequest::getFloat('price',-1);
		if($price == -1){
		...
		if (is_object($menu)) {
			...
			if($type_id == -1){		
				...
				JRequest::setVar('type_id', $type_id_param);				
			}
			...
			if($isFeatured == 0){
				...
				if ($isFeatured == 0) {
					...
					JRequest::setVar('isFeatured', $isFeatured_param);				
				}
			}
			...
			if($isSold == 0){
				...
				if ($isSold == 0) {
					$isSold = $isSold_param;
					JRequest::setVar('isSold', $isSold_param);				
				}
			}
			
			if($orderby == ""){
				$orderby_param = $params->get('orderby', 'a.pro_name');
				JRequest::setVar('orderby', $orderby_param);				
			}
			
			if($ordertype == ""){
				$ordertype_param = $params->get('ordertype', 'desc');
				JRequest::setVar('ordertype', $ordertype_param);				
			}
			
			if($country_id == 0){
				$country_id_param = $params->get('country_id', 0);
				if ($country_id == 0) {
					JRequest::setVar('country_id', $country_id_param);				
				}
			}
			...
			if($state_id == 0){
				...
				if ($state_id == 0) {
					JRequest::setVar('state_id', $state_id_param);				
				}
			}

			//if(($submitCategory == 0) and (count($catIds) == 0)){
	...
	static function listProperties($option,$company_id,$category_ids,$agent_id,$property_type,$keyword,$nbed,$nbath,$isfeatured,$isSold,$nrooms,$orderby,$ordertype,$limitstart,$limit,$favorites,$price,$filterParams,$city_id,$state_id,$country_id,$max_properties,$show_filterform,$agenttype=-1,$tag_id = 0,$min_price=-1,$max_price=-1){
        ...
        $orderbyArray = array('a.pro_name','a.ref','a.id','a.modified','a.price','a.isFeatured','rand()');
        ...
        /*
		if(intval($user->id) > 0){
			$special = HelperOspropertyCommon::adecial();
			if($special){
	...
	static function save($option,$save){
		global $mainframe,$configs,$configClass,$bio,$languages,$lang_suffix;
	...
	function doreportproperty($id){
        ...
        $security_code = JRequest::getVar('security_code','');
        $captcha_str   = JRequest::getVar('captcha_str','');
