...
if(!class_exists('OsSource')){
abstract class OsSource{
	...
	public function renderThumb($path, $width = 280, $height = 150, $isThumb = true) {
		if ($isThumb){
			...
			$thumbnailPath = str_replace(JPATH_ROOT,JURI::base(),$this->_thumbnailPath);
			$thumbnailPath = str_replace("\\","",$thumbnailPath);
			if (file_exists($imagSource)) {	
				if (!file_exists($thumbPath)) {
					//create thumb
					OsImageHelper::createImage($imagSource, $thumbPath, $width, $height, true);
				}
				return $thumbnailPath . $imageName;
				 ...
				 if (!file_exists($thumbPath)){
					 ...
					 if ($this->_CreateImageUsingCurl( $path, $thumbPath, 5 )) {
						 OsImageHelper::createImage($thumbPath, $thumbPath, $width, $height, true);
						 return $thumbnailPath . $imageName;
					 }
				 } else {
					 return $thumbnailPath . $imageName;
