		...
		if ( (!$levelEnd || $level < $levelEnd) && $rows = modOspropertyCategoryMenuHelper::osGetCategories($parent, $params) ) {
			...
			foreach( $rows as $row ) {
				if ($level >= $levelStart) {
					...
					?>
					<li<?php echo ($currentCat == $row->category_id ? ' id="current"' : '') ?>><a class="level<?php echo $level . $menuClass . ($currentCat == $row->category_id ? ' active' : '') ?>" href="<?php echo $link ?>" target="_self"><span><?php echo htmlspecialchars(stripslashes(OSPHelper::getLanguageFieldValue($row,'category_name')), ENT_COMPAT, 'UTF-8') ?> (<?php echo $total?>)</span></a>
                <?php
				}
